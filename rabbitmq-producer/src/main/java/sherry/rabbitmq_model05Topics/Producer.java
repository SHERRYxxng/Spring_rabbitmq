package sherry.rabbitmq_model05Topics;
...
/**
 * @Description: 初学MQ之通配符模式的生产者 通配符模式相对路由模式来说多了路由key中可以添加通配符<br/>
 *                  是分组了根据队列的不同key来给不同的消息<br/>
 *                  首先创建了2个枚举属性用于队列的名字<br/>
 *                  再通过工具类实现了工厂对象的创建和数据源的配置<br/>
 *                  创建频道<br/>
 *                  随后声明了一个  &模糊查询模式  TOPIC 的交换机<br/>
 *                  在创建了2个队列<br/>
 *                  将交换机和队列进行绑定  &同时添加路由过程中可以使用 举例error.#代表所有下级目录 error.*代表下一级目录<br/>
 *                  发布路由发布<br/>
...
public class Producer {
    ...
    public static void main(String[] args) throws Exception {
        ...
        channel.exchangeDeclare("text_TOPIC",
    ...
    //客户端需要跟服务器发送消息
        ...
        channel.queueBind(QUEUE_NAME1,"text_TOPIC","error.#");


        channel.queueBind(QUEUE_NAME2,"text_TOPIC","error.*");
        channel.queueBind(QUEUE_NAME2,"text_TOPIC","info");
        channel.queueBind(QUEUE_NAME2,"text_TOPIC","warning");
        channel.queueBind(QUEUE_NAME2,"text_TOPIC","sherry");
        ...
        //这里配置了只能QUEUE_NAME1才能使用,因为下了三级路由.*只管下一级,.#管下所有级目录
        channel.basicPublish("text_TOPIC","error.sherry.index",null,message.getBytes());
